...
const TickerChange = styled.span`
  color: ${p => p.$up ? '#2ecc71' : p.$down ? '#e74c3c' : '#888'};
  font-size: 12px;
  display: flex;
  align-items: center;
  gap: 2px;
...
const Header = styled.div`
  ...
  margin-bottom: 30px;
  h1 { margin: 0; color: #333; font-size: 28px; font-weight: 700; }
  p { margin: 8px 0 0 0; color: #666; font-size: 16px; }
`;
...
const TabButton = styled.button`
  ...
  &:hover { background: ${p => p.$active ? '#5a6fd8' : '#f8f9fa'}; }
  .count { font-size: 11px; opacity: 0.8; margin-left: 4px; }
`;
...
const StockPrice = styled.div`
  text-align: right;
  .current-price { font-size: 20px; font-weight: 700; color: #333; margin-bottom: 4px; }
  .original-price { font-size: 12px; color: #999; margin-bottom: 2px; }
  .change { font-size: 14px; font-weight: 600; color: ${p => p.$changeColor}; }
`;
...
const StatItem = styled.div`
  .label { font-size: 12px; color: #666; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; }
  .value { font-size: 14px; font-weight: 600; color: #333; }
  .original-value { font-size: 11px; color: #999; margin-top: 2px; }
`;
...
const PageButton = styled.button`
  ...
  &:hover:not(:disabled) { background: ${p => p.$active ? '#5a6fd8' : '#f0f0f0'}; }
  &:disabled { opacity: 0.4; cursor: not-allowed; }
`;

const PageInfo = styled.span`
  font-size: 14px; color: #666; padding: 0 8px;
`;

/* ── Ticker Component ── */

const TickerBanner = ({ indices, exchangeRates }) => {
  const items = [];
  ...
  if (indices) {
    for (const idx of indices) {
      items.push({
        type: 'index',
        name: idx.name,
        value: formatNumber(Math.round(idx.value * 100) / 100),
        change: idx.change,
          ...
          <TickerItem key={i}>
            ...
            {item.type === 'index' && (
              ...
              >
                ...
                {item.change > 0 ? '+' : ''}{formatNumber(Math.round(item.change * 100) / 100)}
                ({item.changePercent > 0 ? '+' : ''}{(item.changePercent || 0).toFixed(2)}%)
              </TickerChange>
            )}
            {item.type === 'fx' && (
              <TickerChange $up={false} $down={false}>원</TickerChange>
            )}
          </TickerItem>
        ))}
      </TickerTrack>
...
const Markets = () => {
  ...
  const renderStockPrice = (stock) => {
    ...
    return (
      <StockPrice $changeColor={getProfitColor(stock.change || 0)}>
        <div className="current-price">{formatStockPrice(stock)}</div>
        {market !== 'KRW' && stock.exchange_rate && (
          <div className="original-price">{priceSym}{formatNumber(stock.current_price)}</div>
        )}
  ...
  const renderPagination = () => {
    ...
    return (
      <Pagination>
        <PageButton disabled={page <= 1} onClick={() => setCurrentPage(1)}>{'<<'}</PageButton>
        <PageButton disabled={page <= 1} onClick={() => setCurrentPage(p => p - 1)}>{'<'}</PageButton>
        {start > 1 && <PageInfo>...</PageInfo>}
        {pages.map(p => (
          <PageButton key={p} $active={p === page} onClick={() => setCurrentPage(p)}>{p}</PageButton>
        ))}
        {end < total_pages && <PageInfo>...</PageInfo>}
        <PageButton disabled={page >= total_pages} onClick={() => setCurrentPage(p => p + 1)}>{'>'}</PageButton>
        <PageButton disabled={page >= total_pages} onClick={() => setCurrentPage(total_pages)}>{'>>'}</PageButton>
  ...
  return (
    <>
      {/* 티커 전광판 */}
      <TickerBanner
        indices={marketData?.market_indices}
        exchangeRates={marketData?.exchange_rates}
      />
      ...
      <Container>
        ...
        {marketListLoading ? (
          ...
          <EmptyState>
            <h3>{searchQuery ? '검색 결과가 없습니다' : '시장 데이터가 없습니다'}</h3>
            <p>{searchQuery ? '다른 검색어를 시도해보세요' : '시장 데이터를 불러올 수 없습니다'}</p>
          </EmptyState>
        )}
      </Container>
    </>
